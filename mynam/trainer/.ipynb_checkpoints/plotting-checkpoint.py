...
def plot_ensemble(dataset: torch.utils.data.Dataset, 
    ...
    out_mean = np.mean(outputs, axis=0)
    figa = plt.figure()
    ax = figa.add_subplot(1, 1, 1)
    ax.plot(X[:, 0], y, '.', label="targeted")
    ax.plot(X[:, 0], out_mean, '.', label="predicted")
...
def plot_preds(dataset: torch.utils.data.Dataset, 
        ...
        """
        in_features = dataset.in_features
        X, y, feature_outs, gen_func_names = dataset.X, dataset.y, dataset.fnn, dataset.gen_func_names
