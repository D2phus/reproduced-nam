...
def train(config: dict,
       ...
       https://pytorch.org/tutorials/beginner/hyperparameter_tuning_tutorial.html
        ...
        model = NAM(config=static_cfg, name="NAM_FINE_TUNE", in_features=len(dataset[0][0]), num_units=static_cfg.num_basis_functions)
        
        optimizer = torch.optim.Adam(model.parameters(), lr=config["lr"], weight_decay=config["decay_rate"]) 
        criterion = lambda nam_out, fnn_out, model, targets: penalized_loss(static_cfg, nam_out, fnn_out, model, targets)
        metrics = lambda nam_out, targets: mae(nam_out, targets) if static_cfg.regression else accuracy(nam_out, targets)
        metrics_name = "MAE" if static_cfg.regression else "Accuracy"
       ...
       # ckpts_dir = os.path.join(config.logdir, "ckpts")
        ...
        num_epochs = static_cfg.num_epochs
        ...
        for epoch in range(static_cfg.num_epochs):
            ...
            # print statistics
            if epoch % static_cfg.log_loss_frequency == 0: 
                print(
